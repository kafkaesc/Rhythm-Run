...
import { GSB_SEARCH_ENDPOINT } from '@/lib/getsongbpm';
...
export async function GET(request: NextRequest) {
	...
	// Extract the requested song title
	...
	uri.searchParams.set('type', 'song');
	uri.searchParams.set('lookup', song);
	...
	if (!res.ok) {
		...
		console.error('GetSongBPM song search failed:', res.status, body);
		return NextResponse.json(
