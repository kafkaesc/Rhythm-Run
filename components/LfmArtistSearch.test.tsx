...
import LfmArtistSearch from './LfmArtistSearch';
...
it('Does not trigger a new search when the state is loading', async () => {
	...
	expect(mockUseLastFmArtistSearch).not.toHaveBeenCalledWith('Rilo Kiley');
	mockUseLastFmArtistSearch.mockReturnValue({
		artists: null,
		loading: null,
		error: null,
...
it('Triggers a search with the entered text on form submit', async () => {
	...
	const searchBtn = screen.getByRole('button', { name: /^search$/i });
	await userEvent.click(searchBtn);
	expect(mockUseLastFmArtistSearch).toHaveBeenCalledWith('Rilo Kiley');
});
